// ignore_for_file: no_logic_in_create_state, use_super_parameters, prefer_interpolation_to_compose_strings
...
class PaginationSearchingState extends State<PaginationSearching> {
  ...
  void initState() {
    showTextField = false;
    // Clear any previous global error message so successful responses can append items
    AppUtils.errorMessage = '';
    // Ensure default parsing for generic lists (id/code/desc)
    Global.commonSearchType = SearchListType.defaultType;
  ...
  Map getData() {
    final String companyId = (Global.empData?.companyId ?? '').toString();
    final String urlLower = widget.url.toLowerCase();
    // Send minimal required keys per endpoint (handle both snake_case and CamelCase variants)
    if (widget.url.endsWith(ApiUrl.commonReferenceSearch) ||
        urlLower.contains('common_reference_list') ||
        urlLower.contains('commonreferencelist')) {
      return {
        'company_id': companyId,
        'txn_type': widget.txnType,
      };
    }
    if (widget.url.endsWith(ApiUrl.getCommonRefTxnCodeList) ||
        urlLower.contains('common_reference_txn_code_list') ||
        urlLower.contains('commonreferencetxncodelist')) {
      return {
        'company_id': companyId,
        'ref_id': widget.refId ?? '',
      };
    }
    if (widget.url.endsWith(ApiUrl.getRefDocNoList) ||
        urlLower.contains('common_reference_doc_no_list') ||
        urlLower.contains('commonreferencedocnolist')) {
      return {
        'company_id': companyId,
        'txn_type': widget.txnType,
    ...
    Map data = {
      ...
      'menu_id':
          widget.isMenuId ? (Global.menuData?.id ?? '5788').toString() : '',
      'charge_type': widget.chargeTypeCode ?? '',
      'ref_txn_type': widget.refTxntype ?? '',
      'ref_id': widget.refId ?? '',
      'emp_id': widget.empId ?? '',
  ...
  Future<void> _fetchPage() async {
    final payload = getData();
    debugPrint('PaginationSearching payload: ' + payload.toString());
    _myRepo.postApi(widget.url, payload).then((value) {
      HseSorCommonSearchingModel data =
          HseSorCommonSearchingModel.fromJson(value);
      debugPrint('PaginationSearching parsed count: ' +
          (data.data?.searchList?.length ?? 0).toString());
